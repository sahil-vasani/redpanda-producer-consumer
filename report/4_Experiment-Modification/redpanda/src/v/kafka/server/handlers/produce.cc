...
static void log_batch_stats(const model::ntp& ntp,
                             const batch_produce_stats& stats) {
    auto ms = std::chrono::duration_cast<std::chrono::microseconds>(
                  stats.duration).count();

    vlog(klog.info,
         "[BATCH-STATS] ntp={} bytes={} records={} latency_us={} ok={}",
         ntp, stats.batch_bytes, stats.record_count, ms, stats.success);
}
  ...
  std::unique_ptr<ss::promise<>> dispatched) {
    ...
    for (const auto& record : req.batch->copy_records()) {
        if (record.has_key()) {
            ...
            auto key_buf = record.key().copy();

            std::string tmp;
            tmp.reserve(key_buf.size_bytes());

            for (auto& frag : key_buf) {
                tmp.append(
                    reinterpret_cast<const char*>(frag.get()),
                    frag.size()
                );
            }
            ...
            vlog(klog.info,
                "✅ NEW KEY ACCEPTED: ntp={} key='{}'",
                req.ntp, key_str);
        }
    }
      // ==================== DUPLICATE DETECTION END ====================

    uint32_t custom_max_batch_bytes = req.batch_max_bytes * 5; //i do changes here ✅ 
