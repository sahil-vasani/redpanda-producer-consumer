...
struct empty_fetch_tracker {
    uint64_t empty_count{0};
    uint64_t total_count{0};

    void record(bool is_empty) {
        total_count++;
        if (is_empty) empty_count++;

        if (total_count % 100 == 0) {
            vlog(klog.info,
                 "[FETCH-EMPTY] total={} empty={} empty_pct={:.1f}%",
                 total_count, empty_count,
                 100.0 * empty_count / total_count);
        }
    }
};   // ✅ THIS ; IS REQUIRED

static thread_local empty_fetch_tracker g_empty_fetch_tracker;
