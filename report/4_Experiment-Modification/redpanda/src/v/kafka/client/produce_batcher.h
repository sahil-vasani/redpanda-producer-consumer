...
public:
    ...
    ss::future<model::record_batch> consume() {
        // NEW
        size_t client_req_count = _client_reqs.size();
        ...
        // NEW LOG
        if (client_req_count > 1) {
            fmt::print(stderr,
                "[BATCH-AGG] aggregated {} client requests into 1 broker batch"
                " ({} records total)\n",
                client_req_count, batch.record_count());
        }
